...
class Tap_TapCheckout_Model_Shared extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getFormFields()
    {
		...
		$post_url=Mage::getStoreConfig('payment/tapcheckout_shared/post_url');
	...
	{ 
	   ...
	   $salt=Mage::getStoreConfig('payment/tapcheckout_shared/salt');
	     $curl = curl_init();
				...
				//echo '<pre>';print_r($response->reference->order);exit;
		   ...
		   if ($response->status == 'CAPTURED') {
				...
				// {
						$comment = 'Tap payment successful.<br/>Tap Charge ID: '.$_GET['tap_id'].'<br/>Payment Type: '.$response->source->payment_method.'<br/>Payment Ref: '.$response->reference->payment;
		   ...
		   Mage::getSingleton('core/session')->addError("Tap payment failed. Please try again");
  ...
  //       else
		...
		//    $order->loadByIncrementId($response->reference->order);
		//    $order->setState(Mage_Sales_Model_Order::STATE_CANCELED, true);
		//   // Invento updated 
